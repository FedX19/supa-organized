  ...
  return (
    <div className="space-y-6 animate-fadeIn">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-white">
              {hasIssues ? `${summary.totalIssues} Issues Found` : 'No Issues Detected'}
            </h2>
            <p className="text-gray-400 mt-1">
              {hasIssues
                ? 'Review the issues below and copy SQL statements to fix them'
                : 'Your database relationships look healthy'}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard
          label="Staff Missing Membership"
        ...
        />
        <StatCard
          label="Players Without Guardians"
...
function IssueCategory({ detail, isExpanded, onToggle, copiedSql, onCopySql }: IssueCategoryProps) {
  ...
  return (
    <div>
      <button
        onClick={onToggle}
        className="w-full px-4 py-4 flex items-center gap-4 hover:bg-dark-surface transition-colors text-left"
      >
        <div className="w-10 h-10 bg-amber-500/20 rounded-lg flex items-center justify-center">
          <span className="text-amber-400 font-bold">{detail.count}</span>
        </div>
